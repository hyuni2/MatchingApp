...
class DBManager(
    ...
    override fun onUpgrade(db: SQLiteDatabase?, oldVersion: Int, newVersion: Int) {
        if (oldVersion < 2) {  // 오류 처리-기존 DB 버전이 낮다면 새 테이블 추가
    ...
    fun getUserLocation(userId: String): UserLocation? {
        ...
        //userId가 UserLocation 테이블에 존재하는지 미리체크
        ...
        if (count == 0) {
            Log.e("DBManager", "사용자 위치 없음: userId = $userId") //위치데이터 없는 경우 로그출력
            return null
        }
        ...
        return if (cursor.moveToFirst()) {
            ...
            cursor.close()
            Log.d("DBManager", "사용자 위치 로드 성공: userId = $userId, lat = $latitude, lng = $longitude") //위치로드 성공 로그
